...
@Component({
  selector: 'app-caseweigher-viewer',
  standalone: true,
  imports: [
    RouterOutlet,
  ...
  templateUrl: './caseweigher-viewer.component.html',
  styleUrl: './caseweigher-viewer.component.scss',
})
export class CaseweigherViewerComponent {
  ...
  async ngOnInit() {
    ...
    this.routeSubstription = this.route.firstChild?.url.subscribe((url: any) => {
      if (url[url.length - 1]?.path === 'demo') {
        ...
        this.caseweigherService.frmGroup.get('timeframe')?.setValue(this.TimeFrameEnum.DateShift);
      } else {
        frm.report = url[url.length - 1]?.path ?? '';
      }
      const reportName = this.homeService.upperCaseFirstLetter(frm.report);
      this.caseweigherService.frmGroup.get('report')?.setValue(reportName);
    });
  ...
  onRefresh() {
    ...
    this.caseweigherService.fetchDB();
  }

  // onFrmGroupChange() {
  //   if (this.caseweigherService.frmGroup.valid) {
  //     const frm = this.caseweigherService.frmGroup.value;
  //     localStorage.setItem('caseweigher.frmGroup', JSON.stringify(frm));
  ...
  get reportname() {
    return this.caseweigherService.frmGroup.get('report')?.value ?? 'Undefined';
  }

  get timeFrame() {
    return this.caseweigherService.frmGroup.get('timeframe')?.value ?? TimeFrame.Live;
  }
  ...
  dateChange(date: Date) {
    ...
    this.caseweigherService.frmGroup.get('toDate')?.setValue(date);
    this.caseweigherService.onFrmGroupChange('timeframe');
  }

  // onRefresh() {
  //   this.homeService.alert.clear();
  //   if (this.homeService.frmGroupHistory.valid) {
  //     const frm: IFrmGroupHistory = this.homeService.frmGroupHistory.value as IFrmGroupHistory;
  //     const url = this.homeService.serverMap.getJsonData(0, frm);
  ...
  onExport() {
    this.homeService.alert.clear();
    const frm = this.caseweigherService.frmGroup.value as IFrmGroupHistory;
    ...
    //     this.caseweigherService.dataSourceScale.data.forEach((e) => {
    //       datetimeframe += `${e.serverName} Cleared at: ${formatDate(e.cleared * 1000, 'M/d/y h:mm:ss a', 'en-US')}\n`;
    //     });
    //   } else {
    //     datetimeframe = `Cleared at: ${formatDate(
    //       this.caseweigherService.dataSourceScale.data[frm.serverIndex].cleared * 1000,
    //       'M/d/y h:mm:ss a',
    //       'en-US'
    //     )}\n`;
    //   }
    ...
    let header = `Checkweigher ${frm.report}` + '\n'; //+ '-' + frm.timeframe.toLocaleLowerCase()
    ...
    this.caseweigherService.exportReportEvent$.next(exportCriteria);
  }
}
