...
import { IRate, IRateRaw } from '../datasource/sizer.model';
...
@Component({
  selector: 'app-sizer-rate',
  imports: [],
  templateUrl: './sizer-rate.component.html',
  styleUrl: './sizer-rate.component.scss',
})
export class SizerRateComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.sizerService.frmGroup.get('report')?.setValue('Rate');
    this.sizerService.disableRefresh.set(true);
    this.sizerService.disableExport.set(true);
    this.sizerService.allowAllServersSelection.set(false);

    // Initialize Chart.js instead of Google Charts
    this.initializeChart();
    // this.chartLoaded = true;
    this.sizerService.disableRefresh.set(false);
    this.sizerService.disableExport.set(false);
    this.loadData();
    ...
    this.formGroupValueChangedSubscription = this.sizerService.frmGroup.get('serverIndex').valueChanges.subscribe((frm: any) => {
      this.drawChart(this.response);
    });
  }
  ...
  async populateData(selectedServerIndex: number = this.sizerService.frmGroup.get('serverIndex')?.value) {
    this.response = this.sizerService.rateResponse.filter((e) => e.serverIndex === selectedServerIndex);
    this.drawChart(this.response);
  }
  ...
  drawChart(chartEvent: IRate[]) {
    ...
    let missingData = this.missingData(this.sizerService.startUnix - 60, rates[0]?.timestamp ?? this.sizerService.stopUnix);
    ...
    if (rates.length > 0) {
      missingData = this.missingData(rates[rates.length - 1].timestamp, this.sizerService.stopUnix);
