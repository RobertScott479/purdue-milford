...
import { ISummaryDistribution } from '../datasource/distribution.model';
...
@Component({
  selector: 'app-distibution-viewer',
  standalone: true,
  imports: [
    RouterOutlet,
    FormsModule,
  ...
  styleUrl: './distribution-viewer.component.scss',
})
export class DistributionViewerComponent {
  ...
  async ngOnInit() {
    const frm = this.distributionService.frmGroup.value;
    ...
    this.routeSubstription = this.route.firstChild?.url.subscribe((url: any) => {
      if (url[url.length - 1]?.path === 'demo') {
        ...
        this.distributionService.frmGroup.get('timeframe')?.setValue(this.TimeFrameEnum.DateShift);
      } else {
        frm.report = url[url.length - 1]?.path ?? '';
      }
      const reportName = this.homeService.upperCaseFirstLetter(frm.report);
      this.distributionService.frmGroup.get('report')?.setValue(reportName);
    });

    await this.homeService.serverMap.loadServerMap(this.mode);
    this.distributionService.onFrmGroupChange();
  ...
  onRefresh() {
    ...
    this.distributionService.fetchDB();
  }

  get reportname() {
    return this.distributionService.frmGroup.get('report')?.value ?? 'Undefined';
  }

  get timeFrame() {
    return this.distributionService.frmGroup.get('timeframe')?.value ?? TimeFrame.Live;
  }
  ...
  dateChange(date: Date) {
    ...
    this.distributionService.frmGroup.get('toDate')?.setValue(date);
    this.distributionService.onFrmGroupChange('timeframe');
  }

  onExport() {
    this.homeService.alert.clear();
    const frm = this.distributionService.frmGroup.value as IFrmGroupHistory;
    ...
    if (frm.timeframe === this.TimeFrameEnum.Live) {
      ...
      //   this.distributionService.dataSourceScale.data.forEach((e) => {
      //     datetimeframe += `${e.serverName} Cleared at: ${formatDate(e.cleared * 1000, 'M/d/y h:mm:ss a', 'en-US')}\n`;
      //   });
      // } else {
      //   datetimeframe = `Cleared at: ${formatDate(
      //     this.distributionService.dataSourceScale.data[frm.serverIndex].cleared * 1000,
      //     'M/d/y h:mm:ss a',
      //     'en-US'
      //   )}\n`;
      // }
    ...
    let header = `${this.distributionService.moduleID} ${frm.report}` + '\n'; //+ '-' + frm.timeframe.toLocaleLowerCase()
    ...
    this.distributionService.exportReportEvent$.next(exportCriteria);
  }
}
